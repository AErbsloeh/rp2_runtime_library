#ifndef FPGA_BASIC_H_
#define FPGA_BASIC_H_
...
/*! \brief Struct fon configuring the Logic on a programmed Field Programmable Gate Array using a SPI interface
...
* \param init_done      Boolean with state of the module initilization
*/
typedef struct {
    spi_rp2_t *spi;
    size_t number_bytes;
...
*/
bool fpga_spi_init(fpga_spi_t *config);
...
*/
bool fpga_spi_reset_do(fpga_spi_t *config, bool enable);
...
* \return                   Boolean with GPIO state of the reset line
*/
bool fpga_spi_reset_cycle(fpga_spi_t *config, uint8_t num_iterations);
...
*/
bool fpga_spi_send_data(fpga_spi_t *config, uint8_t *data, uint8_t *data_rx);
